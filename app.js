...
window.onload = function (e) {
  ...
  request.onsuccess = function (e) {
    const transaction = request.result.transaction(["db.amx"], "read");
    transaction.oncomplete = function (e) {
      const dbAmx = transaction.objectStore("db.amx");
      console.log(typeof dbAmx);
    };
    transaction.onerror = function (e) {
      alert("Unable to fetch data from database. Close and open the app again.");
    };
  };
  request.onerror  = function (e) {
    alert("Unable to open Database. Allow site data from site settings.");
  };
};
window.onbeforeunload = function (e) {
  const request = window.indexedDB.open("v1", 1);
  request.onsuccess = function (e) {
    const transaction = request.result.transaction(["db.amx"], "write");
    transaction.oncomplete = function (e) {
      const dbAmx = transaction.objectStore("db.amx");
      console.log(typeof dbAmx);
    };
...
/*request.onupgradeneeded = function (e) {
  ...
  const objectStore = database.createObjectStore("db");
  objectStore.transaction.oncomplete = function (e) {
    console.log("Database related work completed successfully.");
  };
};*/
